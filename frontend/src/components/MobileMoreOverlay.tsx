...
export function MobileMoreOverlay({ isOpen, onClose, activeTab, setActiveTab, onLogout }: MobileMoreOverlayProps) {
  ...
  return (
    <div 
      ...
      className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-end lg:hidden justify-center transition-all"
    >
      <div 
        id="mobile-more-overlay-content" 
        className="bg-card w-full max-w-md rounded-t-3xl p-6 space-y-4 border-t border-theme shadow-2xl relative block animate-slide-up"
      >
        {/* Header bar */}
        <div className="flex items-center justify-between pb-2 border-b border-theme">
          <h3 className="font-black text-theme text-sm tracking-tight">Explore FitSphere Modules</h3>
          <button 
            onClick={onClose} 
              ...
              <button
                ...
                className={`flex items-center gap-2.5 p-3 rounded-2xl border text-xs font-bold transition-all text-left ${
                  isActive 
                    ...
                    : 'bg-card border-theme text-theme hover:bg-theme-secondary'
                }`}
              >
                <item.icon className={`w-4.5 h-4.5 shrink-0 ${isActive ? 'text-white' : 'text-theme-dim'}`} />
        ...
        <div className="border-t border-slate-100 pt-4 pb-2">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-2xl bg-rose-50 dark:bg-rose-950 hover:bg-rose-100 dark:hover:bg-rose-900 text-rose-600 text-xs font-black transition-colors cursor-pointer"
          >
